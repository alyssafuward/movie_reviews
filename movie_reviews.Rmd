...
Download the ml-latest-small.zip file from "recommended for education and development" under MovieLens Latest Datasets
...
# Preview data
```{r}
links %>% head() %>% formattable(align="l")
movies %>% head() %>% formattable(align="l")
ratings %>% head() %>% formattable(align="l")
tags %>% head() %>% formattable(align="l")
```

# Number of rows
```{r}
data.frame(dataset = c("links", "movies", "ratings", "tags"),
           n = c(links %>% nrow(),
                 movies %>% nrow(), 
                 ratings %>% nrow(),
                 tags %>% nrow())) %>% 
...
# How many ratings per user? Looks like 1 per user
```{r}
all_data %>% 
...
```{r}
all_data2 %>% filter(total_categorized == 0) %>% .$genres %>% unique() %>% head() %>% formattable(align="l")
```
...
```{r}
all_data3 %>% group_by(year, genre) %>% count() %>% arrange(desc(n)) %>% head() %>% formattable(align="l")
```
...
- Mean-centered to standardize coefficients
```{r}
...
# Do rating trends change across time?
```{r}
...
- Mean-centered to standardize coefficients
- Controlling for genre as a random factor
- More info here on [mixed models](http://www.bodowinter.com/tutorial/bw_LME_tutorial2.pdf)
```{r}
